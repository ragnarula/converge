---
name: problem-converge_v3
description: Given a defined problem and its research, work with the user to converge on an approach and produce a specification. Use this skill after problem-definition and problem-research, when the user wants to weigh candidate approaches, pick a direction, and turn it into a spec. Sharpens the user's thinking rather than choosing for them, then writes the specification.
version: 0.1.0
---
...
Read the `problem` and `research` artifacts for the concept from SCS (see the `artifacts_v3` skill). These are the `{problem}` and `{research}` referred to below — load them before Step 1.
...
Use the relevant domain skills (`security_v3`, `api-design_v3`, `distributed-systems_v3`, `data-engineering_v3`, `devops-sre_v3`, `infrastructure_v3`, `low-level-systems_v3`) to validate each option. Point out any clear problems they have, particularly if they're unsolvable, have an impact on resource usage or costs, or could have a negative impact on the user.
...
- Requirements - The key requirements written in EARS format (see the `ears_v3` skill).
...
- Test Plan - Testing strategy for the work. What unit/smoke/integration/e2e/qa etc tests need to be added.
- Documentation - Any docs that should be updated or created. Consider both users and developers.

Aim for a max of 600 lines.

Store the document as the `specification` artifact on the concept in SCS (see the `artifacts_v3` skill). Then fan out relevant domain review subagents using the domain skills above; have each read the `specification` artifact for concept `{concept_name}` from SCS rather than receiving it in the prompt. Let the user know if any major issues surface, and save any fixes as a new revision of the `specification` artifact.
