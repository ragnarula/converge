...
Both this orchestrator and every reviewer subagent it spawns follow the `language_v3` skill for tone and vocabulary in all output, including review reports.
...
> - Every FR is a single EARS statement using one of the canonical patterns (see the `ears_v3` skill). Flag any FR not in EARS shape.
