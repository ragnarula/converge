...
Use the `handbook_v3` skill to read and resolve project conventions before
writing the roadmap.
...
You **MUST** understand project guidelines via the `handbook_v3` skill.
...
Use the `review_v3` skill to perform a **Roadmap Review** of the roadmap
...
`requirements_v3/templates/index.template.md` if it doesn't exist). The
...
- **adr**: roadmap-level decisions (sequencing, what was deferred,
  ...
  Apply the same gate the `adr_v3` skill uses — ADR only if meaningful
