---
name: problem-tasks_v3
description: Break a converged specification into a simple ordered task list for the problem-driven flow. Use this skill after problem-converge has produced a specification and you want demoable, ordered implementation tasks. A simplified take on tasks_v3 — it reviews and fixes the breakdown autonomously, raising to the user only when a fix would conflict with the spec.
version: 0.1.0
---
...
Read the `specification` artifact for the concept from SCS (see the `artifacts_v3` skill). In the problem-driven flow the spec already holds the requirements, design, and test plan. Break it into a `tasks` artifact and store it back in SCS.
...
Follow the `language_v3` skill for tone and vocabulary. Output only the task list.

Read the `specification` artifact for concept `{concept_name}` from SCS (see the `artifacts_v3` skill) — that is the specification to break down.
```

Store the result as the `tasks` artifact on the concept (see the `artifacts_v3` skill).
...
Follow the `language_v3` skill.

Read these from SCS for concept `{concept_name}` (see the `artifacts_v3` skill): the `specification` artifact and the `tasks` artifact (the breakdown to review).
```
...
Keep all other tasks as they are. Follow the `language_v3` skill.

Read these from SCS for concept `{concept_name}` (see the `artifacts_v3` skill): the `specification` artifact and the current `tasks` artifact (the breakdown to fix).
