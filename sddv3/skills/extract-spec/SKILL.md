...
SDD artifacts are stored in SCS via the `scs` MCP server, not local files. Use the `artifacts` skill for the storage contract. Extract Spec writes the `specification` artifact on the concept named `{feature}` (standalone) or `{initiative}/{deliverable-slug}` (roadmap deliverable) — the same artifact `requirements` produces.
