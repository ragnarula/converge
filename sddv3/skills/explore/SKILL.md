...
The appended answers are raw subagent output. Before storing, synthesize them into one coherent document — deduplicate overlapping findings, order them sensibly, and resolve contradictions between answers (or note them). Edit the whole document to follow the `language` skill for tone and vocabulary; do not leave it as a stitched-together transcript.

Store the document as the `research` artifact on the concept in SCS (see the `artifacts` skill). Try to limit it to 600 lines.
