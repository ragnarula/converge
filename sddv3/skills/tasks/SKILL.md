...
SDD artifacts are stored in SCS via the `scs` MCP server, not local files. Use the `artifacts` skill for the storage contract. Tasks writes the `tasks` artifact on the concept named `{feature}` (standalone) or `{initiative}/{deliverable-slug}` (roadmap deliverable).
...
Use the `review` skill for a **Task Breakdown Review** of the `tasks` artifact on the concept.
...
> Think hard.
>
> Fix the following issues in the `tasks` artifact on the concept `{concept_name}` (read it via the `scs` MCP server; use the `artifacts` skill for read/write mechanics), using the `design` and `specification` artifacts on `{concept_name}` as reference.
>
